...
auto solve(const Rules &rules, Updates updates)
{
    ...
    auto is_invalid = [&is_in_order](const auto &update) { return !std::ranges::is_sorted(update, is_in_order); };
    auto fix = [&is_in_order](auto &update) { std::ranges::sort(update, is_in_order); return update; };
    auto get_middle = [](const auto &update) { return update[update.size() / 2]; };

    return fold_left(updates | filter(is_invalid) | transform(fix) | transform(get_middle), size_t{}, std::plus<>{});
}
